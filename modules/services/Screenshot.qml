...
QtObject {
    ...
    property string tempPathBase: "/tmp/rshell_freeze"
    property string cropPath: "/tmp/rshell_crop.png"
            ...
            if (exitCode === 0) {
                try {
                    ...
                    var normalized = rawMonitors.map(m => ({
                                ...
                                activeWorkspace: m.metadata ? {
                                    id: m.metadata.active_workspace
                                } : null
                            }));
    ...
    property Process copyProcess: Process {
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                if (text.length > 0)
                    console.warn("Screenshot Copy Error: " + text);
            }
        }
        onExited: exitCode => {
            if (exitCode !== 0) {
    ...
    function processRegion(x, y, w, h) {
        ...
        if (root.monitors.length > 0) {
            ...
            m = root.monitors.find(mon => {
                ...
                return x >= mon.x && x < (mon.x + logicalW) && y >= mon.y && y < (mon.y + logicalH);
            });
        }
