...
Singleton {
    ...
    // Config path for rctl daemon
    property string configPath: (Quickshell.env("XDG_DATA_HOME") || (Quickshell.env("HOME") + "/.local/share")) + "/rshell/rctl.toml"
    ...
    function dispatch(command) {
        ...
        if (action === "workspace") {
            ...
            if (rawArgs)
                cmdArgs.push(rawArgs);
        } else {
            cmdArgs = ["system", "execute", command];
        }

        let finalCommand = ["rctl"].concat(cmdArgs.filter(x => x !== "" && x !== undefined));
    ...
    property Process ensureConfigDir: Process {
        command: ["mkdir", "-p", (Quickshell.env("XDG_DATA_HOME") || (Quickshell.env("HOME") + "/.local/share")) + "/rshell"]
        running: true
    }

    property Process rctlProcess: Process {
        command: ["rctl", "-c", root.configPath, "daemon"]
        running: true
        stdout: SplitParser {
            onRead: data => {
        ...
        onExited: code => {
            console.warn("rctl daemon exited with code:", code);
        }
    }
    ...
    Timer {
        ...
        interval: 500
        running: true
        onTriggered: rctlSubscribe.running = true
    }
    ...
    property Process rctlSubscribe: Process {
        command: ["rctl", "subscribe"]
        running: false
        stdout: SplitParser {
            onRead: data => {
                if (!data)
                ...
                try {
                    ...
                    console.error("RctlService subscribe JSON parse error:", e);
                }
            }
        }
        onExited: code => {
            console.warn("rctl subscribe exited:", code);
    ...
    Component.onDestruction: {
        ...
        rctlProcess.running = false;
        rctlSubscribe.running = false;
    }
}
