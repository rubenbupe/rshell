...
Item {
    ...
    function createTmuxSession(sessionName) {
        if (sessionName) {
            createProcess.command = ["bash", "-c", `cd "$HOME" && setsid ghostty -e tmux new -s "${sessionName}" < /dev/null > /dev/null 2>&1 &`];
        } else {
            createProcess.command = ["bash", "-c", `cd "$HOME" && setsid ghostty -e tmux < /dev/null > /dev/null 2>&1 &`];
        }
    ...
    function attachToSession(sessionName) {
        attachProcess.command = ["bash", "-c", `cd "$HOME" && setsid ghostty -e tmux attach-session -t "${sessionName}" < /dev/null > /dev/null 2>&1 &`];
